...
import { validateEmail, bcryptHashPassword } from "./helpers/formatAndValidation";
...
router.post("/user", async function (req, res) {
  ...
  if (checkUsername.trim() === "" || checkEmail.trim() === "" || checkPassword.trim() === "" || checkRole.trim() === "") {
    ...
    if (loginUser.length > 0) {
      ...
      if (role == "STAFF" && (level[0].level == isLevel.admin || level[0].level == isLevel.webmaster)) {
...
router.patch("/user", async function (req, res) {
  ...
  const result: any = await show("SELECT * FROM Users WHERE username = ? OR email = ?", [user, user]);
